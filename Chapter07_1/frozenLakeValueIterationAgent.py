...
class Agent:
    ...
    def train(self, num_iterations: int, num_episodes: int) -> None:
        self.get_samples(num_episodes)
        for _ in range(num_iterations):
            self.get_samples(num_episodes=num_episodes)
            self.compute_q_values()
            reward_mean = self.play(num_episodes=num_episodes, render=False)
            if reward_mean >= 0.9:
                break
...
def main() -> None:
    ...
    agent = Agent(env)
    agent.train(num_iterations=100, num_episodes=10)
    agent.play(num_episodes=2)
